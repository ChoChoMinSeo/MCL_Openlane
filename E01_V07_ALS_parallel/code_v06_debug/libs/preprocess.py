...
class Preprocessing(object):
    def __init__(self, cfg, dict_DB):
        ...
        # self.shape_mode = ['l_curve', 'r_curve']
    ...
    def calculate_curvature(self, x):
        ...
        thr = 0.05
        # print(curvature[max_idx])
        if abs(curvature[max_idx])<thr:
            return 0
        elif curvature[max_idx]<0:
            return 1
        else:
            return 2
    ...
    def divide_lane_matrix(self):
        ...
        for i in range(mat_all['x_pts'].shape[1]):
            lane_type = np.concatenate([lane_type,np.array([self.calculate_curvature(mat_all['x_pts'][3:,i].copy())]).reshape(1,1)],axis=1)
        data = dict()
    ...
    def set_data_sampling(self):
        ...
        for mode in self.shape_mode:
            ...
            if mode == 'straight':
                self.sample_idx[mode] = np.int32(np.linspace(0, len(mat['idx'][0]), 12))
            elif mode == 'l_curve':
                self.sample_idx[mode] = np.int32(np.linspace(0, len(mat['idx'][0]), 3))
            elif mode == 'r_curve':
                self.sample_idx[mode] = np.int32(np.linspace(0, len(mat['idx'][0]), 3))
    ...
    def complete_lane_matrix(self):
        ...
        for mode in self.shape_mode:
            ...
            for k in range(0, len(self.sample_idx[mode]) - 1):
                ...
                train_x = self.als.convert_data_to_csr_matrix(mat['x_pts'])
                train_df = self.als.convert_data_to_dataframe(train_x)

                train_z = self.als.convert_data_to_csr_matrix(mat['z_pts'])
                train_z_df = self.als.convert_data_to_dataframe(train_z)
                self.als.ALS(train_df, train_z_df,mat['z_pts'])
