...
class Runner(object):
    ...
    def visualize_gt_3dlanes(self, data, img, path):    
        ...
        # extrinsic = self.recalculate_extrinsic(extrinsic)
        ...
        for i, lane in enumerate(lanes_r_new):
            found = False
            for org_lane in lanes_r_org:
                if np.array_equal(lane,org_lane):
                    found = True
                    break
            if not found:
                merged_idx.append(i)
        ...
        ax4.set_zlim3d(-0.5, 0.5)
        ax4.set_xlabel('x')
        ax4.set_ylabel('y')
        ax4.set_zlabel('z')
        ax4.set_title(f'{len(lanes_r_new)} lanes')
        ax4_2.set_xlim(-10, 10)
        ...
        for i, lane in enumerate(lanes_r_org):
            ...
            ax2.plot(x, y, z, lw=2, c=colors[i%20], alpha=1, label='gt')
            ax2_2.plot(x, y, lw=2, c=colors[i%20], alpha=1, label='gt')
            ...
            ax1.plot(x_2d, y_2d, lw=2, c=colors[i%20], alpha=1, label='gt')
            legend.append(i)
        ax2.legend(legend)
        # legend = list()
        for i, lane in enumerate(lanes_r_new):
